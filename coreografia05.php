        ...
        <form id="myForm">
            ...
            foreach ($files as $file) {
                if ($file !== '.' && $file !== '..' && is_file($dir . $file) && preg_match('/\.(JPG|jpg|jpeg|png)$/', $file)) {
                    echo "<div class='lista-img'> <img src='$dir$file' alt='$file' id='img' class='img watermark'  onclick='toggleCheckbox(this); countSelectedInputs(); contador();'  style='opacity: 1;'> <input type='checkbox' id='meu-checkbox' name='imagens' class='checkbox' value='$file' style='display:none;'><div class='marca-dagua'> <img class='marca_dagua' src='img/2.png' alt=></div>          </div>";
                }
            }
            ?>

            <script>
                const meuCheckbox = document.getElementById("meu-checkbox");

                meuCheckbox.addEventListener("change", function() {
                    localStorage.setItem("checkbox-state", meuCheckbox.checked);
                });
            </script>
            ...
            <script>
                ...
                if (checkboxState) {
                    // Se o valor for verdadeiro, define o estado do checkbox como selecionado
